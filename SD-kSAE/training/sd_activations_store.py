...
class CustomFeatureDataset(Dataset):
    def __init__(self, dataset_dir, layer_dir):
        ...
        """
            # Create a DataLoader with shuffling and batching
        self.activations = []
        self.feature_dir = os.path.join(dataset_dir, layer_dir)
        
        # # Load the cached activations and labels
        # activation_files = sorted([f for f in os.listdir(self.feature_dir)], key=lambda x: int(re.search(r'image_(\d+)_features.npy', x).group(1)))
        # for activation_file in activation_files:
        #     self.activations.append(torch.tensor(np.load(os.path.join(self.feature_dir, activation_file))).clone().detach().mean((-1,-2)))
